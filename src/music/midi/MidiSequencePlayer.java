...
public class MidiSequencePlayer implements SequencePlayer {
    ...
    private static final int DEFAULT_VELOCITY = 100; // the volume
    
    // event callback functions
    private final Map<Integer, Consumer<Double>> callbacks = new HashMap<>();
    
    // next available callback number
    private int nextCallback = 0;
    
    private static final int META_MARKER = 6; // "marker" meta message type
    private static final int META_END_OF_TRACK = 47; // "end_of_track" meta message type
     ...
     * @throws InvalidMidiDataException
     */
    public MidiSequencePlayer(int beatsPerMinute, int ticksPerBeat) throws MidiUnavailableException, InvalidMidiDataException {
    ...
    private int saveCallback(Consumer<Double> callback) {
        callbacks.put(nextCallback, callback);
        return nextCallback++;
    }
     ...
     *         or off a note at a certain tick
     */
    @Override
    public String toString() {
        ...
        for (int i = 0; i < track.size(); i++) {
            ...
            if (msg instanceof ShortMessage) {
                ...
                MetaMessage mmg = (MetaMessage) msg;
